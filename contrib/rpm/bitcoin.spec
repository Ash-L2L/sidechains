...
#man pages
Source20:	https://raw.githubusercontent.com/bitcoin/bitcoin/v%{version}/doc/man/bitnamesd.1
Source21:	https://raw.githubusercontent.com/bitcoin/bitcoin/v%{version}/doc/man/bitnames-cli.1
Source22:	https://raw.githubusercontent.com/bitcoin/bitcoin/v%{version}/doc/man/bitnames-qt.1

#selinux
Source30:	https://raw.githubusercontent.com/bitcoin/bitcoin/v%{version}/contrib/rpm/bitcoin.te
# Source31 - what about bitnames-tx and bench_bitcoin ???
...
The bitnames-cli utility allows you to communicate and control a bitcoin daemon
over RPC, the bitnames-tx utility allows you to create a custom transaction, and
...
mv %{buildroot}%{_bindir}/bitnamesd %{buildroot}%{_sbindir}/bitnamesd

# systemd stuff
mkdir -p %{buildroot}%{_tmpfilesdir}
cat <<EOF > %{buildroot}%{_tmpfilesdir}/bitcoin.conf
d /run/bitnamesd 0750 bitcoin bitcoin -
EOF
...
PID_FILE="/run/bitnamesd/bitnamesd.pid"
EOF
...
[Service]
Type=forking
ExecStart=%{_sbindir}/bitnamesd -daemon -conf=\${CONFIG_FILE} -datadir=\${DATA_DIR} -pid=\${PID_FILE} \$OPTIONS
...
# man pages
install -D -p %{SOURCE20} %{buildroot}%{_mandir}/man1/bitnamesd.1
install -p %{SOURCE21} %{buildroot}%{_mandir}/man1/bitnames-cli.1
%if %{_buildqt}
install -p %{SOURCE22} %{buildroot}%{_mandir}/man1/bitnames-qt.1
%endif
...
%attr(0644,root,root) %{_mandir}/man1/bitnames-qt.1*
%endif
...
%attr(0755,root,root) %{_sbindir}/bitnamesd
...
%attr(0644,root,root) %{_mandir}/man1/bitnamesd.1*

%files utils
%defattr(-,root,root,-)
%license COPYING
%doc COPYING bitcoin.conf.example doc/README.md
%attr(0755,root,root) %{_bindir}/bitnames-cli
%attr(0755,root,root) %{_bindir}/bitnames-tx
%attr(0755,root,root) %{_bindir}/bench_bitcoin
%attr(0644,root,root) %{_mandir}/man1/bitnames-cli.1*
