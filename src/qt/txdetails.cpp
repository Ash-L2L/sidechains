...
void  TxDetails::SetTransaction(const CMutableTransaction& mtx)
{
    ...
    if (tx.nVersion == TRANSACTION_BITNAME_CREATE_VERSION) {
        ui->labelNote->setText("This is a BitName creation transaction.");
    }
...
void TxDetails::AddTreeItem(int index, QTreeWidgetItem *item)
{
    ...
    if (index < 0 || index > INDEX_BITNAMES)
        return;
    ...
    if (!topItem) {
        ...
        if (index == INDEX_BITNAMES)
            topItem->setText(0, "BitNames");
